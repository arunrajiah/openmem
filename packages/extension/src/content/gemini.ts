/**
 * Content script for gemini.google.com (ISOLATED world).
...
window.addEventListener(OPENMEM_EVENT_KEY, (raw) => {
  ...
  if (event.type === "OPENMEM_CAPTURE_ERROR") {
    console.warn("[openmem/gemini] capture error:", event.message);
    return;
  }
  ...
  if (event.type === "OPENMEM_CAPTURED_TURN") {
    ...
    chrome.runtime.sendMessage(msg).catch((err) => {
      console.warn("[openmem/gemini] sendMessage failed:", err);
    });
  }
});
